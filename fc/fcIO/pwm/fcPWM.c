...
uint8_t initPWM(void) {
	...
	for (uint8_t timerCount = 0; timerCount < PWM_TIMER_COUNT; timerCount++) {
		...
		if (err != ESP_OK) {
			sprintf(logBuffer, "PWM , Timer:[%d] , Creation Failed!\n", timerCount);
			logString(logBuffer);
			return 0;
		}
		sprintf(logBuffer, "PWM , Timer:[%d] , Creation Success!\n", timerCount);
		logString(logBuffer);
		...
		for (uint8_t tempOperatorCount = 0; tempOperatorCount < PWM_TIMER_OPERATOR_COUNT; tempOperatorCount++) {
			...
			if (err != ESP_OK) {
				sprintf(logBuffer, "PWM , Timer[%d] , Operator:[%d] , Creation Failed!\n", timerCount, tempOperatorCount);
				logString(logBuffer);
				return 0;
			}
			sprintf(logBuffer, "PWM , Timer[%d] , Operator:[%d] , Creation Success!\n", timerCount, tempOperatorCount);
			logString(logBuffer);

			//Attaching Operator
			err = mcpwm_operator_connect_timer(operators[tempOperatorCount], timers[timerCount]);
			if (err != ESP_OK) {
				sprintf(logBuffer, "PWM , Timer[%d] , Operator[%i] , Association Failed!\n", timerCount, tempOperatorCount);
				logString(logBuffer);
				return 0;
			}
			sprintf(logBuffer, "PWM , Timer[%d] , Operator[%i] , Association Success!\n", timerCount, tempOperatorCount);
			logString(logBuffer);
			...
			for (uint8_t tempGenAndCompCount = 0; tempGenAndCompCount < PWM_TIMER_OPERATOR_GENERATOR_COUNT; tempGenAndCompCount++) {
				...
				if (err != ESP_OK) {
					sprintf(logBuffer, "PWM , Timer[%d] , Operator [%d] , Comparator[%i] , Creation Failed!\n", timerCount, tempOperatorCount, tempGenAndCompCount);
					logString(logBuffer);
					return 0;
				}
				sprintf(logBuffer, "PWM , Timer[%d] , Operator [%d] , Comparator[%i] , Creation Success!\n", timerCount, tempOperatorCount, tempGenAndCompCount);
				logString(logBuffer);
				...
				if (err != ESP_OK) {
					sprintf(logBuffer, "PWM , Timer[%d] ,  Operator [%d] , Generator[%i] , Creation Failed!\n", timerCount, tempOperatorCount, tempGenAndCompCount);
					logString(logBuffer);
					return 0;
				}
				sprintf(logBuffer, "PWM , Timer[%d] ,  Operator [%d] , Generator[%i] , Creation Success!\n", timerCount, tempOperatorCount, tempGenAndCompCount);
				logString(logBuffer);

				setPWMValue(genAndCompCount, 0);
				sprintf(logBuffer, "PWM , Timer[%d] ,  Operator [%d] , Generator[%i] , PWM[%d] , Set to 0 Success!\n", timerCount, tempOperatorCount, tempGenAndCompCount, genAndCompCount);
				logString(logBuffer);
				...
				if (err != ESP_OK) {
					sprintf(logBuffer, "PWM , Timer[%d] ,  Operator [%d] , Generator[%i] , Empty Action Failed!\n", timerCount, tempOperatorCount, tempGenAndCompCount);
					logString(logBuffer);
					return 0;
				}
				sprintf(logBuffer, "PWM , Timer[%d] ,  Operator [%d] , Generator[%i] , Empty Action Success!\n", timerCount, tempOperatorCount, tempGenAndCompCount);
				...
				if (err != ESP_OK) {
					sprintf(logBuffer, "PWM , Timer[%d] ,  Operator [%d] , Generator[%i] , TSH Action Failed!\n", timerCount, tempOperatorCount, tempGenAndCompCount);
					logString(logBuffer);
					return 0;
				}
				sprintf(logBuffer, "PWM , Timer[%d] ,  Operator [%d] , Generator[%i] , TSH Action Success!\n", timerCount, tempOperatorCount, tempGenAndCompCount);
				logString(logBuffer);
		...
		if (err != ESP_OK) {
			sprintf(logBuffer, "PWM , Timer[%i] , Enable Failed!\n", timerCount);
			logString(logBuffer);
			return 0;
		}
		sprintf(logBuffer, "PWM , Timer[%i] , Enable Success!\n", timerCount);
		logString(logBuffer);
	} //Timer Loop
...
uint8_t startPWMs() {
	...
	for (uint8_t timerCount = 0; timerCount < PWM_TIMER_COUNT; timerCount++) {
		...
		if (err != ESP_OK) {
			sprintf(logBuffer, "PWM , Timer[%i] , Start Failed!\n", timerCount);
			logString(logBuffer);
			return 0;
		}
		sprintf(logBuffer, "PWM , Timer[%i] , Start Success!\n", timerCount);
		logString(logBuffer);
	}
	return 1;
}
...
uint8_t stopPWMs() {
	...
	for (uint8_t timerCount = 0; timerCount < PWM_CHANNEL_COUNT; timerCount++) {
		...
		if (err != ESP_OK) {
			sprintf(logBuffer, "PWM , Timer[%i] , Stop Failed!\n", timerCount);
			logString(logBuffer);
			return 0;
		}
		sprintf(logBuffer, "PWM , Timer[%i] , Stop Success!\n", timerCount);
		logString(logBuffer);
	}
	return 1;
}
...
void setPWMValue(uint8_t channel, uint16_t pwmValue) {
	uint16_t scaledPEMValue = PWM_MIN_PULSEWIDTH_US + mapToRange(pwmValue, 0, PWM_PULSEWIDTH_US, 0, PWM_PULSEWIDTH_US);
